document.addEventListener('DOMContentLoaded', async () => {
    ...
    exportHistoryButton.addEventListener('click', () => { /* ... کد خروجی اکسل مثل قبل ... */ });


    // --- ۴. منطق تب ثبت حضور و غیاب (بدون تغییر) ---
    async function initializeRegisterTab() { /* ... کد این بخش مثل قبل است ... */ }
    document.getElementById('attendance-form').addEventListener('submit', async (event) => { /* ... کد این بخش مثل قبل است ... */ });
    ...
    registerTabElements.attendanceForm.addEventListener('submit', async (event) => { event.preventDefault(); registerTabElements.submitButton.disabled = true; registerTabElements.submitButton.textContent = 'در حال ثبت...'; registerTabElements.statusMessage.textContent = ''; const attendanceData = []; const rows = registerTabElements.memberListBody.querySelectorAll('tr'); rows.forEach(row => { const memberId = row.dataset.memberId; const checkedRadio = row.querySelector('input[type="radio"]:checked'); if (memberId && checkedRadio) { attendanceData.push({ memberId: memberId, status: checkedRadio.value }); } }); if (attendanceData.length > 0 && attendanceData.length !== rows.length) { registerTabElements.statusMessage.textContent = 'لطفاً وضعیت تمام اعضا را مشخص کنید.'; registerTabElements.submitButton.disabled = false; registerTabElements.submitButton.textContent = 'ثبت نهایی'; return; } const requestBody = { action: 'saveAttendance', payload: { institutionId: userData.institutionId, data: attendanceData } }; try { const response = await fetch(API_URL, { method: 'POST', body: JSON.stringify(requestBody) }); const result = await response.json(); if (result.status === 'success') { registerTabElements.statusMessage.style.color = 'green'; registerTabElements.statusMessage.textContent = 'حضور و غیاب با موفقیت به‌روزرسانی شد!'; registerTabElements.submitButton.textContent = 'ثبت شد'; } else { registerTabElements.statusMessage.style.color = '#d93025'; registerTabElements.statusMessage.textContent = result.message || 'خطا در ثبت اطلاعات.'; registerTabElements.submitButton.disabled = false; registerTabElements.submitButton.textContent = 'ثبت نهایی'; } } catch(error) { registerTabElements.statusMessage.style.color = '#d93025'; registerTabElements.statusMessage.textContent = 'خطا در ارتباط با سرور.'; registerTabElements.submitButton.disabled = false; registerTabElements.submitButton.textContent = 'ثبت نهایی'; } });
    exportHistoryButton.addEventListener('click', () => { let filteredHistory = [...fullHistory]; if (currentHistoryFilters.status !== 'all') { filteredHistory = filteredHistory.filter(r => r.status === currentHistoryFilters.status); } if (filteredHistory.length === 0) { alert('داده‌ای برای خروجی گرفتن وجود ندارد.'); return; } const dataToExport = filteredHistory.map(record => ({ "تاریخ": record.date, "نام عضو": membersMap[record.memberId] || `(شناسه: ${record.memberId})`, "وضعیت": record.status })); const worksheet = XLSX.utils.json_to_sheet(dataToExport); const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, worksheet, "تاریخچه حضور و غیاب"); XLSX.writeFile(workbook, `History_${userData.username}.xlsx`); });

document.addEventListener('DOMContentLoaded', () => {
    ...
    async function apiCall(action, payload) {
        try {
            ...
            if (!token) {
                // اگر توکن به هر دلیلی پاک شده بود، کاربر را خارج کن
                localStorage.removeItem('userData');
                window.location.href = 'index.html';
                return;
            }
    ...
    function setupEventListeners() {
        ...
        dashboardContainer.addEventListener('click', async (e) => {
            ...
            if (menuButton) { const instId = menuButton.dataset.instId; const menu = document.getElementById(`menu-${instId}`); document.querySelectorAll('.card-menu-dropdown').forEach(m => { if(m.id !== menu.id) m.style.display = 'none'; }); menu.style.display = menu.style.display === 'block' ? 'none' : 'block'; return; }
            ...
            if (actionButton) {
                ...
                if (action === 'edit-user') { openEditModal(instId, username); } 
                else if (action === 'manage-members') { window.location.href = `manage-members.html?id=${instId}&name=${encodeURIComponent(username)}`; } 
                else if (action === 'archive-inst') { if (confirm(`آیا از آرشیو کردن موسسه "${username}" مطمئن هستید؟`)) { const result = await apiCall('archiveInstitution', { institutionId: instId }); alert(result.data ? result.data.message : result.message); if (result.status === 'success') location.reload(); } }
            }
        });
    ...
    // =================================================================
    // بخش ۵: بارگذاری اولیه و رفرش خودکار
    // =================================================================

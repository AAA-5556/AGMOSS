document.addEventListener('DOMContentLoaded', async () => {
    ...
    const memberProfileCard = document.getElementById('member-profile-card'); // عنصر جدید برای کارت

    // ... (متغیرهای وضعیت مثل قبل) ...
    let allRecords = []; 
    let memberNames = {};
    ...
    // --- ۲. توابع نمایش ---
    // (توابع renderDashboard, populateFilters, renderPage, renderTable, renderPagination بدون تغییر هستند)
    function renderDashboard(stats) { /* ... کد قبلی ... */ }
    function populateFilters() { /* ... کد قبلی ... */ }
    function renderPage() { /* ... کد قبلی ... */ }
    function renderTable(records) { /* ... کد قبلی ... */ }
    function renderPagination(totalPages) { /* ... کد قبلی ... */ }

    // --- ۳. منطق فیلترها ---
    function applyAllFilters() { /* ... کد قبلی ... */ }

    // --- ۴. تنظیم Event Listeners ---
    // (شنونده‌های رویداد برای فیلترها و ریست بدون تغییر هستند)
    ...
    statusFilterButtons.forEach(btn => { /* ... کد قبلی ... */ });
    resetFiltersButton.addEventListener('click', () => { /* ... کد قبلی ... */ });

    // --- بخش اصلاح شده: کلیک روی نام عضو ---
    adminDataBody.addEventListener('click', async (e) => {
        if (e.target.classList.contains('clickable-member')) {
            ...
            currentPage = 1;
            renderPage(); // ابتدا جدول را فیلتر می‌کند

            // حالا آمار را از API دریافت و کارت را پر می‌کند
            try {
    ...
    // کدهای کامل توابع بدون تغییر برای جلوگیری از خطا
    function renderDashboard(stats) { dashboardContainer.innerHTML = ''; stats.forEach(stat => { const card = document.createElement('div'); card.className = 'stat-card'; card.innerHTML = `<h3>${stat.name}</h3><p>تعداد کل اعضا: <span class="highlight">${stat.memberCount}</span></p><p>آخرین بروزرسانی: <span class="highlight">${stat.lastUpdate}</span></p><p>آمار آخرین روز: <span class="highlight present">${stat.present} حاضر</span> / <span class="highlight absent">${stat.absent} غایب</span></p>`; dashboardContainer.appendChild(card); institutionNames[stat.id] = stat.name; }); populateFilters(); }
    function populateFilters() { institutionFilter.innerHTML = '<option value="all">همه موسسات</option>'; Object.keys(institutionNames).forEach(id => { const option = document.createElement('option'); option.value = id; option.textContent = institutionNames[id]; institutionFilter.appendChild(option); }); }
    function renderPage() { memberProfileView.style.display = currentFilters.memberId ? 'block' : 'none'; const filteredRecords = applyAllFilters(); const totalPages = Math.ceil(filteredRecords.length / ITEMS_PER_PAGE); currentPage = Math.min(currentPage, totalPages || 1); const startIndex = (currentPage - 1) * ITEMS_PER_PAGE; const endIndex = startIndex + ITEMS_PER_PAGE; const pageRecords = filteredRecords.slice(startIndex, endIndex); renderTable(pageRecords); renderPagination(totalPages); }
    ...
    async function initializeAdminPanel() { loadingMessage.textContent = 'در حال بارگذاری...'; try { const [dashboardResult, adminDataResult, ...memberResults] = await Promise.all([ fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'getDashboardStats' }) }).then(res => res.json()), fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'getAdminData' }) }).then(res => res.json()), ...[1, 2, 3, 4, 5].map(id => fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'getMembers', payload: { institutionId: id } }) }).then(res => res.json())) ]); if (dashboardResult.status === 'success') { renderDashboard(dashboardResult.data); } memberResults.forEach(res => { if (res.status === 'success') { res.data.forEach(member => { memberNames[member.memberId] = member.fullName; }); } }); if (adminDataResult.status === 'success') { allRecords = adminDataResult.data.sort((a, b) => new Date(b.date.split('،')[0].trim().replace(/\//g, '-')) - new Date(a.date.split('،')[0].trim().replace(/\//g, '-'))); renderPage(); } loadingMessage.style.display = 'none'; } catch (error) { console.error('خطا در بارگذاری پنل مدیر:', error); loadingMessage.textContent = 'خطا در ارتباط با سرور.'; } }
